...
export default function Akun({ className }) {
  return (
    <Menu as="div" className={`relative ${className && className}`}>
      {({ open }) => (
        <>
          <Menu.Button className="inline-flex items-center w-full justify-center rounded-md pr-1.5 sm:pr-3 font-semibold text-gray-500 hover:text-gray-700 dark:text-neutral-400 dark:hover:text-neutral-200 focus:outline-none">
            Admin
