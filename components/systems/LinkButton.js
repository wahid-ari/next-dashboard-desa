...
export default function LinkButton({ className, href, children, ...rest }) {
  return (
    <Link href={href} {...rest} className={`${className ? className + " " : ""
      }text-sm transition-all px-3 py-1.5 rounded font-medium bg-blue-700 dark:bg-sky-600 hover:bg-blue-800 dark:hover:bg-sky-700 text-white focus:ring-2 focus:ring-sky-400`}>
      {children}
    </Link>
  );
}
