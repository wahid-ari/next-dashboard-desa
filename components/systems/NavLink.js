...
NavLink.logout = ({ href, icon, className, children }) => {
  return (
    <Link
      passHref
      href={href}
      className={`${className} flex w-full items-center justify-start gap-3 rounded px-4 py-2 text-sm text-red-800 transition-all hover:bg-red-100 dark:text-red-500 dark:hover:bg-neutral-800 dark:hover:text-red-400`}
    >
      {icon}
      <span>{children}</span>
    </Link>
